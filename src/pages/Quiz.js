...
const Quiz = ({ quiz, onBack, onFinish }) => {
  ...
  const handleSubmitQuiz = useCallback(() => {
    ...
    const timeSpent = Math.round((quiz.timeLimit * 60 - timeRemaining) / 60); // in minutes
    ...
    if (state.isAuthenticated) {
      dispatch({ type: 'ADD_QUIZ_HISTORY', payload: quizResult });
    }

    dispatch({ type: 'SET_QUIZ_RESULTS', payload: quizResult });
  }, [quiz, selectedAnswers, totalQuestions, timeRemaining, state.user, state.isAuthenticated, dispatch]);
      ...
      <div className="p-4">
        <Card className="shadow">
          ...
          <Card.Body className="text-center p-5">
            <div className="mb-4">
              <div className={`display-1 text-${getScoreColor(result.score)} mb-3`}>
                {result.score}%
              </div>
              <h4 className={`text-${getScoreColor(result.score)}`}>
        ...
        <Card.Header className="bg-primary text-white">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              ...
              <small>{subjects.find(s => s.id === quiz.subjectId)?.name}</small>
            </div>
