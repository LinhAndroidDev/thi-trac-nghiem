...
const HistoryView = () => {
  ...
  const loadHistory = async () => {
    try {
      setLoading(true);
      const historyData = QuizController.getQuizHistory();
          ...
          {selectedQuiz && (
            <div>
              ...
              {selectedQuiz.answers.map((answer, index) => {
                const question = QuizController.getQuizById(selectedQuiz.quizId)?.questions.find(q => q.id === answer.questionId);
