...
const HomeView = ({ onQuizStart, onNavigate }) => {
  ...
  const loadHomeData = async () => {
    try {
      setLoading(true);
      const homeData = QuizController.getHomeData();
      setData(homeData);
    } catch (error) {
  ...
  const handleSubjectClick = (subject) => {
    ...
    const quizzes = QuizController.getQuizzesBySubject(subject.id);
              ...
              >
                <Card.Body>
                  ...
                  <Badge bg="secondary" className="mt-2">
                    {QuizController.getQuizzesBySubject(subject.id).length} bài thi
                  </Badge>
                </Card.Body>
              </Card>
            </Col>
